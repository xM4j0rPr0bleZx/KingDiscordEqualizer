...
function readUiSettings() {
  try {
    return JSON.parse(localStorage.getItem('kings-equalizer-ui') || '{}');
  } catch {
    return {};
  }
}

function saveUiSettings(preferences) {
  try {
    localStorage.setItem('kings-equalizer-ui', JSON.stringify(preferences));
  } catch {
    // The switches still work when browser storage is unavailable.
  }
}

const uiPreferences = readUiSettings();

function setUiItem(key, visible, save = true) {
  const item = uiItems[key];
  if (!item) return;
  item.classList.toggle('ui-hidden', !visible);
  const input = settingInputs.find(control => control.dataset.uiSetting === key);
  if (input) input.checked = visible;
  uiPreferences[key] = visible;
  if (save) saveUiSettings(uiPreferences);
  if (key === 'nowPlaying' && visible) updateTitleOverflow();
}

for (const input of settingInputs) {
  const key = input.dataset.uiSetting;
  setUiItem(key, uiPreferences[key] !== false, false);
  input.addEventListener('change', () => setUiItem(key, input.checked));
}

saveUiSettings(uiPreferences);
...
showAllSettings.addEventListener('click', () => {
  for (const key of Object.keys(uiItems)) setUiItem(key, true, false);
  saveUiSettings(uiPreferences);
});

document.addEventListener('pointerdown', event => {
  if (!settingsPanel.hidden && !settings.contains(event.target)) setSettingsOpen(false);
});
